...
import com.sidetrack.auth.AuthState
        ...
        modifier = baseModifier
            ...
            .onPreviewKeyEvent { keyEvent ->
                if (keyEvent.nativeKeyEvent.keyCode == KeyEvent.KEYCODE_DPAD_CENTER ||
                    ...
                    // Consume both DOWN and UP: Button's own clickable modifier also
                    // triggers a click on ENTER/DPAD_CENTER key-up, so only swallowing
                    // DOWN here let every press fire onClick() twice — the second one
                    // landing on whatever now has focus (e.g. the Cancel button on the
                    // screen this same click just navigated to).
