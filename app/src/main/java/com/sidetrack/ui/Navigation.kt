...
import com.sidetrack.auth.ZeroconfState
...
fun SidetrackNavigation(
    ...
    Log.i("SidetrackAuth", "startDestination=$startDestination authState=$authState")
    ...
    DisposableEffect(mainActivity, showNowPlaying) {
        ...
        if (!showNowPlaying) playerViewModel.exitSeekMode()
        onDispose {
            mainActivity?.isNowPlayingVisible = false
    ...
    LaunchedEffect(authState.isAuthenticated, authState.version, authState.isLoading, state.isConnected) {
        Log.i("SidetrackAuth", "auto-connect: auth=${authState.isAuthenticated} version=${authState.version} loading=${authState.isLoading} connected=${state.isConnected}")
        if (authState.isAuthenticated && !state.isConnected && !authState.isLoading) {
            if (authManager.needsReauth()) {
                Log.i("SidetrackAuth", "auto-connect: needsReauth=true, logging out")
                authManager.logout()
    ...
    LaunchedEffect(state.isConnected, currentRoute) {
        Log.i("SidetrackAuth", "isConnected changed: ${state.isConnected} currentRoute=$currentRoute")
        if (state.isConnected) {
            libraryViewModel.initApi(authManager)
            libraryViewModel.loadPlaylists()
            if (currentRoute == Routes.LOGIN) {
                Log.i("SidetrackAuth", "navigating from login to library (connected)")
    ...
    LaunchedEffect(authState.isAuthenticated, currentRoute) {
        if (currentRoute != null) {
            if (!authState.isAuthenticated && currentRoute != Routes.LOGIN) {
                Log.i("SidetrackAuth", "auth-lost: navigating to login, currentRoute=$currentRoute")
                navController.navigate(Routes.LOGIN) {
                    popUpTo(0) { inclusive = true }
                }
            } else if (authState.isAuthenticated && state.isConnected && currentRoute == Routes.LOGIN) {
                Log.i("SidetrackAuth", "auth-gained: player already connected, navigating to library")
                navController.navigate(Routes.LIBRARY) {
                    popUpTo(Routes.LOGIN) { inclusive = true }
                }
            }
    ...
    DynamicSidetrackTheme(albumColors = albumColors, einkMode = settingsState.einkMode) {
                ...
                NavHost(
                    ...
                    composable(Routes.LOGIN) {
                        ...
                        LoginScreen(
                            ...
                            onPairWithSpotifyApp = {
                                authManager.startZeroconfPairing("Sidetrack")
                            },
                            onCancelPairing = {
                                authManager.cancelZeroconfPairing()
                            },
                    ...
                    composable(Routes.SETTINGS) {
                        SettingsScreen(
                            ...
                            onKeyMappingClick = { navController.navigate(Routes.KEY_MAPPING) },
                        )
                    }

                    composable(Routes.KEY_MAPPING) {
                        KeyMappingScreen(
                            keyMappingManager = keyMappingManager,
                            onBack = { navController.popBackStack() },
                        )
